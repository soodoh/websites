...
DiLoreto's CloudFront-to-Amplify migration is soaking at `Native`: production uses the Amplify-managed apex, `www`, and `paul` mappings, while the former custom CloudFront distribution and supporting resources remain retained without aliases for rollback. Its deployment smoke checks the public release marker, clean paths, custom 404 status/body, caching, security headers, and domain redirects. When Amplify's native `301` clean-URL canonicalization is observed, it also validates the canonical location and final custom `404`. Keep using `apps/diloreto/infra/amplify-hosting-transition.yml` through the soak and rollback window; final cleanup must use a separately reviewed, approval-gated change set as described in [`../apps/diloreto/infra/hosting-architecture.md`](../apps/diloreto/infra/hosting-architecture.md).
