...
const roots = [".", "apps/sarabeth-studio", "apps/portfolio-website", "apps/carolyn-portfolio", "apps/diloreto-website", "apps/carolyn-portfolio/infra"];

function audit(missingRoot?: string, missingType = "dependencies") {
	...
	try {
		...
		const result = Bun.spawnSync([process.execPath, join(import.meta.dir, "audit-installed-ranges.ts"), base, output]);
...
for (const type of ["dependencies", "devDependencies"]) {
	test(`absent app direct ${type} is reported and fails`, () => {
		const result = audit("apps/sarabeth-studio", type);
		expect(result.exitCode).toBe(1);
		expect(result.evidence["apps/sarabeth-studio"].missing).toEqual([
			{ parent: "apps/sarabeth-studio", type: "direct", dep: "websites-audit-missing-direct", range: "1.0.0" },
		]);
	});
