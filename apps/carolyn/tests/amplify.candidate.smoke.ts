...
test.describe("Isolated Amplify candidate behavior", () => {
	test.beforeEach(async ({ context }) => {
		// No canonical/legacy/CDN HTTP, including redirects, images or server functions.
		await context.route("**/*", (route) =>
			routeCandidateRequest(route, canonicalOrigin),
		);
	});
	test("serves public routes and assets", async ({ request }) => {
		...
		const rootHtml = await (await request.get("/", { maxRedirects: 0 })).text();
		const assetPath = rootHtml.match(/src="(\/assets\/[^"]+\.js)"/)?.[1];
		expect(assetPath).toBeDefined();
		if (assetPath) {
			expect((await request.get(assetPath, { maxRedirects: 0 })).status()).toBe(
				200,
			);
		}
	});
	...
	test("loads photography albums through a server function", async ({
		...
		// External images are intentionally blocked; production smoke retains decode checks.
	});
	...
	test("serves the exact candidate root release without production requests", async ({
		...
		expect(await getRootReleaseCommit(request, canonicalOrigin)).toBe(
