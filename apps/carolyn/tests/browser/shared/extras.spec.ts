...
import { expectDarkSurfaceFocus } from "@tests/visual-helpers";

test.describe("Route recovery", () => {
	test("retries a failed route loader", {
		tag: "@desktop-only",
	}, async ({ diagnostics, page }) => {
		diagnostics.allowConsoleError(/Failed to load resource:.*503/);
		diagnostics.allowConsoleError(/Seroval Error/);
		diagnostics.allowConsoleError(/Unhandled route error/);
...
test.describe("Header route state", () => {
	...
	test("renders a home route error with an opaque header", async ({
		diagnostics,
		page,
	}) => {
		diagnostics.allowConsoleError(/Failed to load resource:.*503/);
