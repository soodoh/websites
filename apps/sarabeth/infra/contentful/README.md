...
OpenTofu owns the Contentful webhook that dispatches the existing Sarabeth GitHub Actions deployment workflow. CloudFormation continues to own Amplify Hosting and the encrypted S3 state bucket.
...
Production applies run through `.github/workflows/configure-contentful-sarabeth.yml`. The `production-sarabeth` GitHub Environment supplies:
