terraform {
  ...
  backend "s3" {
    bucket       = "websites-sarabeth-contentful-tofu-state-015989770400-us-west-2"
...
variable "contentful_space_id" {
  description = "Sarabeth's production Contentful space ID."
  type        = string
}
...
resource "contentful_webhook" "github_deployment" {
  ...
  name   = "Sarabeth production deployment"
  url    = "https://api.github.com/repos/soodoh/websites/actions/workflows/deploy-sarabeth.yml/dispatches"
  active = true
