...
- `bun run test:smoke`: run Playwright smoke tests against `dist/client`.
- `bun run check`: run lint, build, output assertions, and smoke tests.
...
- Smoke tests live in `tests/` and run against the production `dist/client` output, not the Vite development server.
