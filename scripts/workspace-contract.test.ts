...
const jsonc = (path: string) =>
	JSON.parse(read(path).replace(/,\s*([}\]])/g, "$1"));
...
describe("workspace contract", () => {
		...
		for (const app of apps) {
			...
			expect(
				requiredScripts.every(
					(script) => typeof local.scripts[script] === "string",
				),
			).toBe(true);
			...
			expect(json("package.json").scripts[`verify:${app}`]).toBe(
				`turbo run ci:verify --filter=${packageNames[app]} --concurrency=1`,
			);
			expect(jsonc("bun.lock").workspaces[`apps/${app}`].name).toBe(
				packageNames[app],
			);
			expect(existsSync(resolve(root, `apps/${packageNames[app]}`))).toBe(
				false,
			);
			...
			expect(
				json(`apps/${app}/tsconfig.json`).compilerOptions.paths["@/*"],
			).toEqual(["./src/*"]);
			for (const directory of ["src", "tests", "infra"]) {
				expect(existsSync(resolve(root, `apps/${app}/${directory}`))).toBe(
					true,
				);
			}
			for (const legacyDirectory of [
				...
				expect(
					existsSync(resolve(root, `apps/${app}/${legacyDirectory}`)),
				).toBe(false);
			}
		}
		expect(existsSync(resolve(root, "apps/carolyn/infra/package.json"))).toBe(
			false,
		);
	...
	test("uses strict uncached tasks and serial complete verification", () => {
		...
		expect(json("package.json").scripts["ci:verify"]).toContain(
			"--concurrency=1",
		);
